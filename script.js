...
function init() {
  ...
  if (mobileMedia.addEventListener) {
    mobileMedia.addEventListener("change", handleDeviceChange);
  } else {
    mobileMedia.addListener(handleDeviceChange);
  }
...
function handleDeviceChange() {
  const changed = applyDeviceLayout();
...
function applyDeviceLayout() {
  const touchMobile = navigator.maxTouchPoints > 0 && window.innerWidth <= 980;
  const isMobile = mobileMedia.matches || touchMobile;
  const nextDevice = isMobile ? "mobile" : "desktop";

  if (document.body.dataset.device === nextDevice) {
    return false;
  }
